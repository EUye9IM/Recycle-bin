...
static void _err_type_double(ConstStr &err, const Argument *arg) {
	err.append("Argument '");
	if ((nullptr != arg->long_name.cstr() && arg->long_name.cstr()[0] != 0))
		err.append(arg->long_name.cstr());
	else {
	...
	err.append("' requires an decimal value.\n");
}
				...
				if (_checkIfArgumentIsMatchRule(the_arg, p_rule)) {
					...
					if (p_rule->type == Type::FLAG) {
						...
						case Type::DEC: {
							...
							int ret;
							char c;
							ret = sscanf(val_str, "%lf%c", &val, &c);
							if (1 != ret) {
								_b_sucess = false;
								_err_type_double(_error_str, p_rule);
							} else
								temp_val.Dec = val;
						} break;
		...
		if (pointer->is_required) {
			if (nullptr != pointer->long_name.cstr()) {
				...
				case Type::INT:
					...
					break;
				case Type::DEC:
					fprintf(out_stream, " -%c {decimal}", pointer->short_name);
					break;
